...
import static ru.read.reader.fb2format.Image.fromBase64ToImage;
...
public class OpenHandler
    ...
    public void characters(char[] ch, int start, int length) {
        switch (lastAtribute) {
            case ("lang"):
                if (isTitleInfo) {
                    titleInfo.setLang(new String(ch, start, length));
                }
            case ("p"):
                if (isAnnatation) {
        ...
        switch (qName) {
            ...
            case ("binary"):
                if (isCoverPage) {
                    ...
                    fromBase64ToImage(pathToImage, imgBase64);
            ...
            case ("title-info"):
                ...
                directory = new File(Main.tempDirectory, description.getTitleInfo().getBookTitle());
                directory.mkdir();
                break;
            case ("author"):
                isAuthor = false;
                ...
                break;
            case ("FictionBook"):
                if (!Main.fictionBookList.contains(ficbook)) {
                    ficbook.setIndex(Main.fictionBookList.size());
